...
def display_data(request):
    # 獲取主表數據
    response1 = requests.get('http://127.0.0.1:8000/api/gettable1')
    ...
    for item in data1:
        ...
        # 獲取該製令單號的詳細資料
        response2 = requests.get(f'http://127.0.0.1:8000/api/gettable2?s={item["製令單號"]}')
...
def remove_item(request, order_id):
    if request.method == 'POST':
        # 調用 API 刪除指定項目
        response = requests.post('http://127.0.0.1:8000/api/removetable1', data={'ids': order_id})
